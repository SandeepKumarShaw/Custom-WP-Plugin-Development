            ...
            <?php else: ?>
                <table class="wp-list-table widefat fixed">
                    ...
                    <?php foreach ($subscriptionForms as $sf): ?>
                        <?php
                        $plans_label = null;
                        $sf_plans = json_decode( $sf->plans );
                        if ( json_last_error() == JSON_ERROR_NONE ) {
                            $plans_label = implode( ', ', $sf_plans );
                        }
                        ?>
                        <tr>
                            <td><?php echo esc_html( $sf->name ); ?></td>
                            <td><?php echo esc_html( $sf->formTitle ); ?></td>
                            <td><?php echo esc_html( $plans_label ); ?></td>
                            <td>
    ...
    <div class="" id="plans">
        ...
        <?php else: ?>
            <table class="wp-list-table widefat fixed">
                <thead>
                <tr>
                    <th><?php esc_html_e( 'ID', 'wp-full-stripe' ); ?> / <?php esc_html_e( 'Display Name', 'wp-full-stripe' ); ?></th>
                    <th><?php esc_html_e('Amount', 'wp-full-stripe'); ?> / <?php esc_html_e('Interval', 'wp-full-stripe'); ?></th>
                    <th><?php esc_html_e('Trial', 'wp-full-stripe'); ?> / <?php esc_html_e('Duration', 'wp-full-stripe'); ?></th>
                    <th><?php esc_html_e('Action', 'wp-full-stripe'); ?></th>
                </tr>
                </thead>
                <tbody id="plansTable">
                <?php foreach ($plans['data'] as $plan): ?>
                    <tr>
                        <td>
                            <b><?php echo esc_html( stripslashes( $plan->id ) ); ?></b><br>
                            <a href="<?php echo admin_url( 'admin.php?page=fullstripe-edit-plan&plan=' . urlencode( $plan->id ) ); ?>"><?php echo esc_html( $plan->name ); ?></a>
                        </td>
                        <td>
                        ...
                        <td>
                            <a class="button button-primary" href="<?php echo admin_url( "admin.php?page=fullstripe-edit-plan&plan=" . urlencode( $plan->id ) ); ?>">Edit</a>
                            <button class="button delete" data-id="<?php echo esc_attr( $plan->id ); ?>" data-type="subscriptionPlan" data-confirm="true">Delete</button>
                        </td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
                ...
                <table class="form-table">
                    ...
                    <tr valign="top">
                        ...
                        <td>
                            ...
                            <p class="description">Which subscription plans can be chosen on this form. The list can be reordered by using drag'n'drop.</p>
                            <input type="hidden" name="plan_order" value="<?php echo urlencode( json_encode( $plan_order ) ); ?>"/>
                        </td>
                    </tr>
                    <tr valign="top">
                        <th scope="row">
            ...
            <table class="form-table">
	            ...
	            <tr valign="top">
		            ...
		            <td>
			            ...
			            <p class="description">You could specify the number of charges after which the subscription is cancelled. Set to 0 to let the subscription run forever.</p>
		            </td>
	            </tr>
