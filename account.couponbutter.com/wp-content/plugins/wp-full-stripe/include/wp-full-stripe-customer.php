...
class MM_WPFS_Customer {
	...
	function fullstripe_payment_charge() {
		...
		if ( $valid ) {
			...
			try {
				...
				$this->db->fullstripe_insert_payment( $result, $address, $stripeCustomer->id, $email, $formId, /* formType */
					'payment' );
				...
				if ( $sendPluginEmail && $sendReceipt == 1 && isset( $_POST['fullstripe_email'] ) ) {
					$this->mailer->send_payment_email_receipt( $email, $amount, $result['source']['name'], $address, $productName, $customInputValues );
				}
	...
	function fullstripe_subscription_charge() {
		...
		if ( $valid ) {
			...
			try {
				...
				$return = array(
					'success' => true,
					'msg'     => __( 'Payment Successful. Thanks for signing up!-check your email now. ', 'wp-full-stripe' )
				);
				if ( $doRedirect == 1 ) {
					if ( $redirectToPageOrPost == 1 ) {
						if ( $redirectPostID != 0 ) {
	...
	function fullstripe_checkout_charge() {
		...
		$billingAddressCountry = isset( $_POST['billing_address_country'] ) ? sanitize_text_field( $_POST['billing_address_country'] ) : null;
		...
		try {
			...
			$result   = $this->stripe->charge_customer( $stripeCustomer->id, $amount, $description, $metadata );
