...
if ($memberID)
{
    ...
    else
    {
        try
        {
            ...
            $currencySymbol = MM_WPFS_Members::getInstance()->get_currency_symbol();
        }
        catch (Exception $e)
        {
            $valid = false;
...
<?php if (!$valid): ?>
    ...
    <div class="error"><p>There was an error retrieving the member data. Please make sure your Stripe API keys are set in WP Full Stripe Settings.</p></div>
<?php else: ?>
    <h3>Member Summary</h3>
    ...
    <table class="form-table">
        ...
        <tr valign="top">
            ...
            <td>
                ...
                {
                    $intervalStr = ucfirst($subscription->plan->interval) . 'ly';
                }
                else
                {
                    $intervalStr = 'per ' . $subscription->plan->interval_count . ' ' . $subscription->plan->interval . 's';
                }

                echo $currencySymbol . sprintf('%0.2f', $subscription->plan->amount / 100.0) . ' ' . $intervalStr;  ?>
                <p class="description"><?php echo $subscription->plan->name; ?></p>
            </td>
        </tr>
        <tr valign="top">
            <th scope="row">
        ...
        <table class="form-table">
            <tr valign="top">
                ...
                <td>
                    ...
                    <p class="description">WARNING: This will update the members role and associated subscription and make a charge to their credit card if necessary. The member will then be billed for this new Role in future.</p>
                </td>
            </tr>
        </table>
        <p class="submit">
            <button type="submit" class="button button-primary" <?php echo (defined('WPFS_MEMBERS_DEMO')) ? 'disabled="disabled"' : '' ?> >Change Membership Level</button>
    ...
    <form action="" method="POST" id="wpfs_members_cancel">
        ...
        <table class="form-table">
            <tr valign="top">
                ...
                <td>
                    <p class="description">This will cancel the members subscription. They will still have access until the end of their last payment period. </p>
                </td>
            </tr>
        </table>
        <p class="submit">
            <button type="submit" class="button button-primary" <?php echo $subscription->cancel_at_period_end ? 'disabled="disabled"' : '' ?> <?php echo (defined('WPFS_MEMBERS_DEMO')) ? 'disabled="disabled"' : '' ?> >Cancel Membership</button>
            <img src="<?php echo plugins_url('/img/loader.gif', dirname(__FILE__)); ?>" alt="Loading..." class="delete_showLoading"/>
        </p>
    </form>
    <hr/>
    <form action="" method="POST" id="wpfs_members_change_role">
        ...
        <table class="form-table">
            <tr valign="top">
                ...
                <td>
                    ...
                    <p class="description">This only changes the members Role, therefore changing their access to your protected content.
                        <strong>THIS WILL NOT CHANGE THE MEMBERS SUBSCRIPTION.</strong> The member will continue to be billed for the role they subscribed to even if you choose a different role here.
        ...
        <p class="submit">
            <button id="wpfs_members_block_access" <?php echo (defined('WPFS_MEMBERS_DEMO')) ? 'disabled="disabled"' : '' ?> class="button button-primary">Change Role</button>
            <img src="<?php echo plugins_url('/img/loader.gif', dirname(__FILE__)); ?>" alt="Loading..." class="change_showLoading"/>
        </p>
    </form>
