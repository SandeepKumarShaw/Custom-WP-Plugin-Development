                ...
                <tr>
                    ...
                    <td class="wpfs-right-align">
                        ...
                        if ( $subscription->plan->interval === 'week' ) {
                            /* translators: 1: currency symbol/code, 2: amount, 3: inverval count > 1 */
                            $price_label = sprintf( _n( '%1$s%2$.2f Weekly', '%1$s%2$.2f per %3$d weeks', $subscription->plan->interval_count, 'wp-full-stripe-members' ), $currencySymbol, $subscription->plan->amount / 100.0, $subscription->plan->interval_count );
                        } else if ( $subscription->plan->interval === 'month' ) {
                            /* translators: 1: currency symbol/code, 2: amount, 3: inverval count > 1 */
                            $price_label = sprintf( _n( '%1$s%2$.2f Monthly', '%1$s%2$2f per %3$d months', $subscription->plan->interval_count, 'wp-full-stripe-members' ), $currencySymbol, $subscription->plan->amount / 100.0, $subscription->plan->interval_count );
                        } else if ( $subscription->plan->interval === 'year' ) {
                            /* translators: 1: currency symbol/code, 2: amount, 3: inverval count > 1 */
                            $price_label = sprintf( _n( '%1$s%2$.2f Yearly', '%1$s%2$.2f per %3$d years', $subscription->plan->interval_count, 'wp-full-stripe-members' ), $currencySymbol, $subscription->plan->amount / 100.0, $subscription->plan->interval_count );
                        }
            ...
            <div class="_100 wpfs-members-summary">
                ...
                <form action="" method="POST" id="wpfs_members_change_level">
                    ...
                    <table class="wpfs-members-table">
                        <tr>
                            <td>
                                <label for="wpfs_members_level"><?php _e('Select New Level', 'wp-full-stripe-members'); ?>:</label>
                                    ...
                                    <?php foreach ($stripePlans as $role => $plan): ?>
	                                    <?php if ( $role !== $member->role ): ?>
		                                    <option
			                                    ...
			                                    <?php MM_WPFS_Members::echo_translated_label( $roleNames[ $role ] ); ?>
		                                    </option>
	                                    <?php endif; ?>
                                    <?php endforeach; ?>
	                                <?php if ( $subscription->cancel_at_period_end ): ?>
		                                <option
			                                ...
			                                data-currency="<?php echo $currencySymbol; ?>">
			                                <?php MM_WPFS_Members::echo_translated_label( $roleNames[ $member->role ] ); ?><?php _e( '(Re-join)', 'wp-full-stripe-members' ); ?>
		                                </option>
	                                <?php endif; ?>
                                </select>
            ...
            <form action="" method="POST" id="wpfs_members_update_card">
                ...
                <table class="wpfs-members-table" id="wpfs_members_update_card_section" style="display: none;">
                    ...
                    <tr>
                        ...
                        <td><input type="text" class="input-mini" autocomplete="off" size="4" maxlength="4" data-stripe="cvc"></td>
                    </tr>
                    <tr>
                        <td>&nbsp;</td>
                        <td>
                            ...
                            <img src="<?php echo plugins_url('/img/loader.gif', dirname(__FILE__)); ?>" alt="Loading..." class="update_showLoading"/>
                        </td>
                    </tr>
                </table>
            </form>
                ...
                <form action="" method="POST" id="wpfs_members_cancel">
                    ...
                    <table class="wpfs-members-table">
                        <tr>
                            <td>
                                <p>
                                    <small><?php _e('You will not be charged again and your subscription will remain active until the end of the current billing period.', 'wp-full-stripe-members'); ?></small>
                                </p>
                            </td>
                            <td class="wpfs-right-align">
