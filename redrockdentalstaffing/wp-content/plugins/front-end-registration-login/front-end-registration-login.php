		...
		<form id="custom_reg_log_login_form"  class="custom_reg_log_form"action="" method="post">
			<fieldset>
				<p>
					...
					<input name="custom_reg_log_user_login" id="custom_reg_log_user_login" class="required" type="text" required/>
				</p>
				<p>
					<label for="custom_reg_log_user_pass">Password</label>
					<input name="custom_reg_log_user_pass" id="custom_reg_log_user_pass" class="required" type="password" required/>
				</p>
				<p>
...
function custom_get_related_listing() {
		...
		if( $permission[0] == 'Enable' ) { //Need admin permission to view user list
			...
			<!--====================PROVIDER APPLICATION UPLOAD SECTION START================-->
				...
				if(!$user_creditapp || !$user_offcaggr){
				    ...
				    echo '<p class="sub-title">Browse the available candidates below, and click on "INQUIRE NOW."  We will reach out to check the candidate&#39;s availability and set up the interview. <a class"red_test" href="'.get_permalink(502).'#service_matches" >View our Fee Schedule</a></p>';
	...
	//print_r($available_search_field);
			...
			if($user_role == 'job_seeker'){
				$output =  '<p class="success">'.$regis_status.'<div class="job-listing-hld">Please wait for authorization to view job list.</div></p>';	
			}
			if($user_role == 'job_provider'){
			...
			<tr>
				...
				<td class="seekers_details">
					<ul>
						...
						<li><span>Max working distance(Miles): </span><?php echo get_user_meta($u_id,'max_working_distance',true)?get_user_meta($u_id,'max_working_distance',true):'N/A'; ?></li>
